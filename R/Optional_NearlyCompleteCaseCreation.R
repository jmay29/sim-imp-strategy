...
dfTraits$trait_count <- apply(dfTraits[, traits], MARGIN = 1, function(x) sum(!is.na(x)))

# Create empty list to hold subsetted dataframes (recommend including at least 3 traits for imputing).
l_dataframes <- CreateNamedList(listLength = length(3:max(dfTraits$trait_count)), elementNames = 3:max(dfTraits$trait_count))

# Loop over different trait missingness thresholds.
for(t in max(dfTraits$trait_count):3){
  
  print(paste("Results for", t, "trait minimum:", sep = " "))
  # Subset to only include rows with data for at least t traits.
  dfSubset <- dfTraits[dfTraits$trait_count >= t,]
